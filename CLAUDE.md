...
ユーザー向けの変更をリリースする前に、[docs/deploy.md の「リリース前の更新情報確認」](docs/deploy.md#リリース前の更新情報確認-必須)を実施してください。

- 対象月の `content/info/*.md` の本文と `summary` に、リリース内容が含まれていることを確認する
- `updated` を実装日やマージ日ではなく、実際のリリース日時へ更新する
- `make generate-info` を実行し、生成結果を確認する
- 本番リリースPRをマージする前に、修正済みの更新情報を `staging` へマージする

`updated` はユーザーごとのお知らせ未読判定に使われるため、更新漏れはリリース通知の欠落につながります。
...
### ⚠️ 日付に関する重要な注意
実装ログのディレクトリ名には、固定の年ではなく実際の実装日を使用してください。
- 正しい例: `2026-08-25/01_機能名.md`（実装日が2026年8月25日の場合）
- 日付が不明な場合は、作成前に現在日を確認してください

### ログ作成のルール

- `yyyy-mm-dd/n_機能名.md` の形式でファイルを作成してください
  - nは連番であり、01から始めてください
  - yyyy-mm-ddは実際の実装日付です
  - 例: `2026-08-25/01_GCSアップロード機能.md`
- ログには以下の内容を含めてください：
  - 実装した機能の概要
  - 特に注意した点や工夫した点
  - テスト結果や動作確認の内容
