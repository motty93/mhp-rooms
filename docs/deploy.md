...
ユーザー向けの変更をリリースする場合は、`main` から `staging` へのリリースPRを作成する前に、
対象月の `content/info/*.md` を確認・更新してください。

1. 本文に、そのリリースでユーザーが利用できるようになる変更がすべて記載されていることを確認する
2. frontmatter の `summary` に主要な変更が過不足なく含まれていることを確認する
3. frontmatter の `updated` を、実装日やマージ日ではなく、実際のリリース日時へ更新する
4. `make generate-info` を実行する
5. `static/generated/info/articles.json` で対象記事の本文、`summary`、`updated` を確認する

`updated` は表示日時だけでなく、お知らせの未読判定にも使われます。ユーザーごとの
`user_notification_states.info_read_at` より `updated` が新しい場合に未読として扱われるため、古い日時のまま
リリースすると、その日時以降にお知らせを開いたユーザーへ新しい更新情報が未読表示されません。

アプリケーションのビルド時にも更新情報は生成されますが、`make generate-info` はリリース前の入力内容と生成結果を
検証するために実行します。生成ファイルそのものはコミット対象ではありません。

ステージングへのマージ後に記載漏れや日時の誤りへ気付いた場合は、`staging` を起点に修正ブランチを作り、
本番リリース前に修正を `staging` へマージしてください。本番リリースPRでは、更新情報の本文、`summary`、
`updated` の確認結果をチェック項目へ含めます。
