...
## 概要
HuntersHubは、モンスターハンターシリーズのマルチプレイヤーセッション管理を行うWebアプリケーションです。クリーンアーキテクチャの原則に基づいて設計されています。
...
│   │   │   └── supabase/         # Supabase統合
│   │   └── persistence/          # データ永続化
│   │       ├── postgres/         # PostgreSQL実装（開発環境）
│   │       ├── turso/            # Turso実装（本番環境）
│   │       └── factory.go        # データベース実装選択
│   ├── models/                    # データモデル
│   │   ├── user.go
│   │   ├── room.go
│   │   └── ...
...
- **モデル** (`internal/models/`)
  ...
  - GORMタグによるマッピング
  - バリデーションルール
  - **BaseModel継承**: 全モデルで共通のID、CreatedAt、UpdatedAtフィールドを継承
...
- **永続化** (`internal/infrastructure/persistence/`)
  - データベース接続管理
  - マイグレーション実行
  - **マルチデータベース対応**: PostgreSQL（開発）とTurso（本番）のFactory パターン実装
